...
class ImageTransforms(object):
    ...
    def __call__(self, img):
        ...
        lr_img = hr_img.resize((int(hr_img.width / self.scaling_factor), int(hr_img.height / self.scaling_factor)),
                               Image.BICUBIC)
        # hr_img = lr_img.resize((int(hr_img.width), int(hr_img.height)), Image.BICUBIC)
...
class ImageTransformsEval(object):
    """
    Image transformation pipeline.
    """
    ...
    def __call__(self, img):
        ...
        # Convert the LR and HR image to the required type
        lr_img1 = convert_image(lr_img, source='pil', target=self.lr_img_type)
        lr_img_orig = convert_image(lr_img, source='pil', target='[-1, 1]')
        hr_img = convert_image(hr_img, source='pil', target='[-1, 1]')

        return lr_img1, hr_img, lr_img_orig
